...
int addResRecord(unsigned char *reader, ResRecord *rr) {
  ...
  strcpy((char *)name, (const char *)rr->name);
  switch (rr->resource->type) {
  case Q_T_A:
    return addResRecord_A(reader, (const char *)name, rr->resource->ttl,
                          (const char *)rr->rdata);
  case Q_T_CNAME:
    return addResRecord_CNAME(reader, (const char *)name, rr->resource->ttl,
                              (const char *)rr->rdata);
  case Q_T_MX:
    return addResRecord_MX(reader, (const char *)name, rr->resource->ttl, 5,
                           (const char *)rr->rdata);
  case Q_T_NS:
    return addResRecord_NS(reader, (const char *)name, rr->resource->ttl,
                           (const char *)rr->rdata);
  case Q_T_PTR:
    return addResRecord_PTR(reader, (const char *)name, rr->resource->ttl,
                            (const char *)rr->rdata);
  }
}

int searchFromCache(unsigned char *buf, DNS_Packet *packet,
                    GHashTable *tables) {
    ...
    if (g_hash_table_contains(table, query->name)) {
      ...
      if (query->question->qtype == Q_T_MX) {
        ...
        additionals = (GList *)g_hash_table_lookup(
            table, ((ResRecord *)answers->data)->rdata);
      }
    }
  }
  if (answers == NULL) {
  ...
  while (1) {
    ...
    if (IS_RECURSIVE) {
      ...
      if (dnsHeader->rcode == 0 &&
          ntohs(dnsHeader->answerCount) != 0) { // 缓存成功的查询
      ...
      if (tld_packet.Header->rcode == 0 &&
          ntohs(tld_packet.Header->answerCount) == 0) {
        printf("正向别的服务器查询\n");
        root_addr_udp.sin_addr.s_addr =
            *(long *)tld_packet.Additional_RRs[0].rdata;
      } else {
        if (tld_packet.Header->rcode == 0 &&
...
int main(void) {
  ...
  strcat(cachefilename, g_get_current_dir());
  strcat(cachefilename, "/data/local.cache");
  CACHE_TABLES = readResRecords(cachefilename);
  CACHE_FP = fopen(cachefilename, "a+");
  if (CACHE_FP == NULL) {
    printf("打开缓存文件失败\n");
  }
